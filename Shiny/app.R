...
B2_sf <- st_transform(B2_sf, crs = '+proj=longlat +datum=WGS84 +no_defs')
...
server <- function(input, output, session) {
  ...
  color.gradient <- colorRampPalette(c("#006d2c","#fed976","#de2d26"))(100) 
  
  
  # low_colors <- colorRampPalette(c("#a1d99b", "#006d2c"))(100)     # Light to dark green
  # median_colors <- colorRampPalette(c("#fed976", "#fd8d3c"))(100)   # Light to dark orange
  # high_colors <- colorRampPalette(c("#fc9272", "#de2d26"))(100) 
  # 
  # low_median_colors <- colorRampPalette(c("#006d2c", "#fed976"))(100)  # Dark green to light orange
  # median_high_colors <- colorRampPalette(c("#fd8d3c", "#fc9272"))(100)
  
  low_colors <- color.gradient[1]   
  low_median_colors <- color.gradient[25] 
  median_colors <- color.gradient[50]
  median_high_colors <- color.gradient[75]
  high_colors <- color.gradient[100]
  
  all.colors <- c(low_colors,low_median_colors,median_colors,median_high_colors,  high_colors)
  ...
  Input.temp <- eventReactive(input$generate,{
    ...
    if (input$type=='count'){
      ...
      B2_sf$color <- cut(prob, breaks = breaks,labels=all.colors, include.lowest = TRUE)
      
      B2_sf <- B2_sf %>%
        group_by(prob.bin) %>%
        mutate(
          certainty = scales::rescale(neg.prob.sd.std, to = c(1, 100))
        ) %>%ungroup()
      ...
      df.prob.councils.2$color <- cut(prob, breaks = breaks ,labels=all.colors, include.lowest = TRUE)
    ...
    if (input$type=='burn_area'){
      ...
      # weibull
      prob.mark.mat <- sapply(1:200, function(x) pweibull(thres, shape = hyper.ba[[x]], scale = param.ba[[x]][grid.month.idx], lower.tail=FALSE))
      prob.z.mat <- sapply(1:200, function(x) param.z[[x]][grid.month.idx])
      prob.mat <- prob.z.mat*prob.mark.mat
    ...
    if (input$type=='joint'){
      ...
      prob.mark.mat1 <- sapply(1:200, function(x) ppois(thres.cnt, param.cnt[[x]][grid.month.idx], lower.tail=FALSE))
      # weibull
      prob.mark.mat2 <- sapply(1:200, function(x) pweibull(thres.ba, shape = hyper.ba[[x]], scale = param.ba[[x]][grid.month.idx], lower.tail=FALSE))
      prob.z.mat <- sapply(1:200, function(x) param.z[[x]][grid.month.idx])
  ...
  observeEvent(input$portugalMap_shape_click, {
    ...
    output$zoomedPlot <- renderPlot({
      ...
      B2_sf_input <- st_intersection(B2_sf_input, council_data)
      ggplot(council_data)+
        geom_sf(data=B2_sf_input, aes(fill = prob.bin),alpha=B2_sf_input$certainty/100,lwd = 0.01) +
        scale_fill_manual(values=c('Low'=all.colors[1],'Low_Median'=all.colors[2],
                                   'Median'=all.colors[3], 'Median_High'=all.colors[4],
                                   'High'=all.colors[5]))+
        geom_sf(alpha=0) +
        ...
        labs(fill  = "Prob Bins",title=paste('Probability in',selected_council ))
    })
  })
}

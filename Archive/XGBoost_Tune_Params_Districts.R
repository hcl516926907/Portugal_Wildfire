...
plot(results_df_pois_6$nrounds,results_df_pois_6$mean_loss_train,type='l',col='blue',ylim=c(0,1))
...
TrainSet2 <- xgb.DMatrix(data=as.matrix(data.rf[data.rf$y>0, c(covar.names)]),label=data.rf[data.rf$y>0, c('log_ba')]
                         # weight =2*as.numeric(data.rf[data.rf$y>0,'log_ba']>5) + 1
                         # weight = rep(1, sum(data.rf$y>0))
                         )
#
set.seed(1234)
model.ba <- xgb.train(params = params_ba, data = TrainSet2, nrounds = params_ba$nrounds)
summary(predict(model.ba,TrainSet2))

mean((predict(model.ba,TrainSet2)-data.rf[data.rf$y>0, c('log_ba')])^2)
...
data.fit[data.fit$y>0,]%>%mutate(square_loss=(log_ba-score_ba)^2)%>%
  group_by(year)%>% summarize(avg_loss=mean(square_loss))


data.fit[data.fit$y>0,]%>%mutate(square_loss=(y-score_cnt)^2)%>%
  group_by(year)%>% summarize(avg_loss=mean(square_loss))

data.fit.dist.pred <- data.fit[,c('NAME_1','time.idx','score_z','score_ba','score_cnt')]
colnames(data.fit.dist.pred) <- c('NAME_1','time.idx','score_z.dist','score_ba.dist','score_cnt.dist')
save(data.fit.dist.pred, file=file.path(dir.out, 'districts_prediction.RData'))

...
data.fit[data.fit$year <= 2022, 'score_ba'] <- exp(data.rf$score_ba)
data.fit[data.fit$year > 2022, 'score_ba'] <- exp(predict(model.ba, as.matrix(data.fit[data.fit$year > 2022,c(covar.names)])))
...
data.rf$score_cnt <- kfold_cv_pred_pos(data.rf,'y',covar.names, params_cnt, 4)
...
data.fit[data.fit$y>0,]%>%mutate(loss=loss_eval.egp(log(score_ba),log_ba,xi,kappa,alpha))%>%
  group_by(year)%>% summarize(avg_loss=mean(loss))
...
save(data.fit, file=file.path(dir.out, 'XGBoost_Score_Council.RData'))
